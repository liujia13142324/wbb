...
(function($){
    $.fn.open = function(anim, time){
        ...
        if(_this.hasClass("hidden")){ 
            ...
            var flag = true;
            _this.one('mozAnimationEnd MSAnimationEnd oanimationend webkitAnimationEnd', function(){
				_this.removeClass(anim);
                _this.removeClass("shown");
			    flag = false; // 如果运行的话，下面的setTimeout不走。代码在这里运行。
			});
			//定时器去掉心跳类
			setTimeout(function () {
			    if(flag != false) {
			    	_this.one('animationend',function () {
			    		_this.removeClass(anim);
		                _this.removeClass("shown"); // 如果上面不支持的话，则运行这里。
			        })
			    }
			},10)
			...
			setTimeout(function () {
			    if(flag != false) {
			    	_this.one('animationend',function () {
			    		 _this.hide();
		            	 _this.removeClass(anim).removeClass("hidden");
		                 des && _this.remove();// 如果上面不支持的话，则运行这里。
			        })
			    }
			},10)
